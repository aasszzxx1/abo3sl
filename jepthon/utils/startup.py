...
async def startupmessage():
    ...
    try:
        if BOTLOG:
            Config.CATUBLOGO = await jepiq.tgbot.send_file(
                ...
                caption="⌯︙**بــوت ابو عسل يـعـمـل بـنـجـاح**  ✅ \n⌯︙**قـنـاة الـسـورس**  :  @vvvs7cvv",
                buttons=[(Button.url("كروب ابو عسل", "https://t.me/vvvs7cvvSupport"),)],
            )
    except Exception as e:
        LOGS.error(e)
        return None
    ...
    try:
        if msg_details:
            ...
            text = (
                message.text
                + "\n\n**⌯︙اهلا وسهلا لقد قمت باعاده تشغيل بـوت ابو عسل تمت بنجاح**"
            )
...
async def add_bot_to_logger_group(chat_id):
    ...
    try:
        ...
        try:
            ...
            LOGS.error(str(e))
#by @vvvs7cvv بس اشوفك خامطه للكود اهينك وافضحك 

jepthon = {"@vvvs7cvv", "@vvvs7cvvSupport", "@Story_lMl10l"}
async def saves():
...
async def verifyLoggerGroup():
    ...
    if BOTLOG:
        ...
        descript = "- عزيزي المستخدم هذه هي مجموعه الاشعارات يرجى عدم حذفها  - @vvvs7cvv"
        photobt = await jepiq.upload_file(file="JepIQ/razan/resources/start/Jepthon.jpg")
        _, groupid = await create_supergroup(
            "مجموعة اشعارات ابو عسل ", jepiq, Config.TG_BOT_USERNAME, descript, photobt
        )
