...
async def startupmessage():
    ...
    try:
        if BOTLOG:
            Config.CATUBLOGO = await jepiq.tgbot.send_file(
                BOTLOG_CHATID,
                "https://telegra.ph/file/679768b8bad8de3138780.jpg",
                caption="⌯︙**بــوت كريزون يـعـمـل بـنـجـاح**  ✅ \n⌯︙**قـنـاة الـسـورس**  :  @Krize_One1",
                buttons=[(Button.url("كروب كريزون", "https://t.me/Krize_one"),)],
            )
    except Exception as e:
        LOGS.error(e)
        return None
    ...
    try:
        if msg_details:
            ...
            text = (
                message.text
                + "\n\n**⌯︙اهلا وسهلا لقد قمت باعاده تشغيل بـوت كريزون تمت بنجاح**"
            )
...
async def add_bot_to_logger_group(chat_id):
    ...
    try:
        ...
        try:
            ...
            LOGS.error(str(e))
#by @Krize_One1 بس اشوفك خامطه للكود اهينك وافضحك 

jepthon = {"@Krize_One1", "@Krize_One", "@jepthon"}
async def saves():
...
async def verifyLoggerGroup():
    ...
    if BOTLOG:
        ...
        descript = "- عزيزي المستخدم هذه هي مجموعه الاشعارات يرجى عدم حذفها  - @Krize_One1"
        photobt = await jepiq.upload_file(file="JepIQ/razan/resources/start/Jepthon.png")
        _, groupid = await create_supergroup(
            "مجموعة اشعارات كريزون ", jepiq, Config.TG_BOT_USERNAME, descript, photobt
        )
