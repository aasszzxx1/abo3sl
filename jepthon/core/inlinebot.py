...
def main_menu():
    text = f"**- [سـورس كريزون](https://t.me/Krize_One1)\
        \n المسـاعد\
        \n\
        \n◽ كريزون لـ {mention}**"
    buttons = [
...
async def inline_handler(event):  # sourcery no-metrics
    ...
    if query_user_id == Config.OWNER_ID or query_user_id in Config.SUDO_USERS:
        ...
        if query.startswith("jepiq"):
            buttons = [
                (
                    ...
                    Button.url(" Jepthon UsᴇʀBoᴛ", "https://t.me/Krize_One1"),
                )
            ]
            ...
            result = builder.article(
                title="رسـالة تـرول من سـورس كريزون 🧸♥",
            ...
            result = builder.article(
                title="همسـة سـرية من سـورس كريزون 🧸♥",
                text=f"فقـط {razan} هـو مـن يستـطيع مشـاهدتهـا !",
                buttons=buttons,
            )
            await event.answer([result] if result else None)
            ...
            result = builder.article(
                title="رسـالة مخفيـة من سـورس كريزون 🧸♥",
                text=f"✖✖✖",
                buttons=buttons,
            )
            await event.answer([result] if result else None)
        ...
        buttons = [
            (
                Button.url("قنـاة السـورس ⚒️", "https://t.me/Krize_One1"),
                Button.url(
                    "كـروب المـساعدة 📬",
                    "https://t.me/Krize_One11",
                ),
            )
        ]
        markup = event.client.build_reply_markup(buttons)
        ...
        text, msg_entities = await event.client._parse_message_text(
            "**[بـوت كريزون 🧸♥](https://t.me/Krize_One1)**\
            \n\
            \n❤ كريزون هـو بـوت بسـيط يدخـل الـى حسـابك لجعـلك تتحـكم به معـ اوامـر معينـة.\
            \n\
            \n**اذا كـنت تـرغب بتـنصيـب السـورس عـلى حسـابك ,\
            \n🐾 تعال الـى [كروب المسـاعدة](https://t.me/Krize_One11)!**",
            "md",
        )
        result = types.InputBotInlineResult(
            id=str(uuid4()),
