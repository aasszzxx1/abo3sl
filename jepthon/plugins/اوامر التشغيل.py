...
async def _(event):
    "Restarts the bot !!"
    if BOTLOG:
        await event.client.send_message(BOTLOG_CHATID, "**⌔︙ابو عسل العرب ↻** \n" "**⌯︙تم تحديث السورس بنجاح ✅ ↻**")
    lMl10l = await edit_or_reply(
        event,
        "**⌯︙جـاري تحديث سورس ابو عسل ، قـد يستغـرق الأمـر 2-3 دقائـق لاتقم بكتابة تحديث مـره اخـرى انتـظـر ⏱**",
    )
    try:
