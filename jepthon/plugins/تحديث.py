...
async def print_changelogs(event, ac_br, changelog):
    changelog_str = (
        f"**⌔︙قام مطورين السورس بتحديث كريزون**\n⌔︙**التـغييرات\n** {changelog}"
    )
...
async def update(event, repo, ups_rem, ac_br):
    ...
    jasme = await event.edit(
        "** ⌔︙تم تحديث سورس كريزون بنجاح انتظر قليلا سوف نخبرك بعد اعادة التشغيل !**"
    )
...
async def upstream(event):
    ...
    event = await edit_or_reply(event, "**⌔︙يـتـم البـحـث عـن تـحديثـات سـورس كريزون انـتـظـر**")
    ...
    if changelog == "" and not force_update:
        await event.edit(
            "**⌔︙سورس كريزون محدث الى اخر اصدار **\n"
            f"**قـنـاة سـورس كريزون** : @Krize_One1"
        )
        return repo.__del__()
    if conf == "" and not force_update:
        ...
        return await event.respond(
            f"⌔ :  لتحديث سورس كريزون ارسل : `.تحديث الان` "
        )
    ...
    if conf == "الان":
        await event.edit("** ⌔︙جار تحـديـث سـورس كريزون انـتـظـر قـليـلا 🔨**")
