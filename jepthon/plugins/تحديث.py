...
async def print_changelogs(event, ac_br, changelog):
    changelog_str = (
        f"**⌯︙قام مطورين السورس بتحديث ابو عسل**\n⌯︙**التـغييرات\n** {changelog}"
    )
...
async def update(event, repo, ups_rem, ac_br):
    ...
    jasme = await event.edit(
        "** ⌯︙تم تحديث سورس ابو عسل بنجاح انتظر قليلا سوف نخبرك بعد اعادة التشغيل !**"
    )
...
async def upstream(event):
    ...
    event = await edit_or_reply(event, "**⌯︙يـتـم البـحـث عـن تـحديثـات سـورس ابو عسل انـتـظـر**")
    ...
    if changelog == "" and not force_update:
        await event.edit(
            "**⌯︙سورس ابو عسل محدث الى اخر اصدار **\n"
            f"**قـنـاة سـورس ابو عسل** : @vvvs7cvv"
        )
        return repo.__del__()
    if conf == "" and not force_update:
        ...
        return await event.respond(
            f"⌔ :  لتحديث سورس ابو عسل ارسل : `.تحديث الان` "
        )
    ...
    if conf == "الان":
        await event.edit("** ⌯︙جار تحـديـث سـورس ابو عسل انـتـظـر قـليـلا 🔨**")
