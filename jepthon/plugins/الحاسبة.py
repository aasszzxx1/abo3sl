...
async def icalc(e):
    if e.client._bot:
        return await e.reply(
            "**الحـاسبة العـلمية لسـورس ابو عسل\n @vvvs7cvv**", buttons=lst
        )
...
async def inlinecalc(event):
    ...
    if (
        ...
        calc = event.builder.article(
            "Calc", text="**الحـاسبة العـلمية لسـورس ابو عسل\n @vvvs7cvv**", buttons=lst
        )
...
async def _(e):
    ...
    lst = list(zip(tultd[::4], tultd[1::4], tultd[2::4], tultd[3::4]))
    lst.append([Button.inline("=", data="calc=")])
    await e.edit("**الحـاسبة العـلمية لسـورس ابو عسل\n @vvvs7cvv**", buttons=lst)
